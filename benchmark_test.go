...
func (suite *BenchmarkSuite) SetUpSuite(c *C) {
	...
	suite.queue1k = CreateQueue(redisHost, redisPort, redisPassword, redisDB, "teststuff1k", false)
	suite.queue4k = CreateQueue(redisHost, redisPort, redisPassword, redisDB, "teststuff4k", false)
	...
	for i := 0; i < 4; i++ {
		q := CreateQueue(redisHost, redisPort, redisPassword, redisDB, "teststuff1k", false)
	...
	for i := 0; i < 4; i++ {
		q := CreateQueue(redisHost, redisPort, redisPassword, redisDB, "teststuff4k", false)
		c, err := q.AddConsumer("c" + strconv.Itoa(i))
		if err != nil {
			panic(err)
		}
