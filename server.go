...
func NewServer(redisHost, redisPort, redisPassword string, redisDb int, port string) *Server {
	observer := NewObserver(redisHost, redisPort, redisPassword, redisDb, false)
	s := &Server{
		port:     port,
		observer: observer,
	}
